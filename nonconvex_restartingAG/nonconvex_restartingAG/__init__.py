...
def _update_smooth_grad_SCAD_LM(N, X, beta_md, y, _lambda, a):
    ...
    '''
    return _update_smooth_grad_convex_LM(N=N, X=X, beta_md=beta_md, y=y) + SCAD_concave_grad(x=beta_md, lambda_=_lambda, a=a)
...
def _update_smooth_grad_MCP_LM(N, X, beta_md, y, _lambda, gamma):
    ...
    '''
    return _update_smooth_grad_convex_LM(N=N, X=X, beta_md=beta_md, y=y) + MCP_concave_grad(x=beta_md, lambda_=_lambda, gamma=gamma)
...
def _eval_obj_SCAD_LM(N, X, beta_md, y, _lambda, a, x_temp):
    ...
    '''
    error = y - X@x_temp
    return (error.T@error)/(2.*N) + _np.sum(SCAD(x_temp, lambda_=_lambda, a=a))
...
def _eval_obj_MCP_LM(N, X, beta_md, y, _lambda, gamma, x_temp):
    ...
    '''
    error = y - X@x_temp
    return (error.T@error)/(2*N) + _np.sum(SCAD(x_temp, lambda_=_lambda, gamma=gamma))
...
#     y_temp /= _np.std(y)
    ...
    eliminated = _np.hstack((_np.array([False]), eliminated)) # because intercept coefficient is not penalized
    return eliminated

@_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def UAG_LM_SCAD_MCP(design_matrix, outcome, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2., L_convex=1.1, add_intercept_column = True):
    '''
...
#     add design matrix column for the intercept, if it's not there already
    if add_intercept_column == True:
        if _np.any(X[:,0] != X[0,0]): # check if design matrix has included a column for intercept or not
...
#         L = _np.max(_np.array([L_convex, 1./(a-1)]))
        ...
        while ((not converged) or (k<3)) and k <= maxit:
            ...
            smooth_grad = _update_smooth_grad_SCAD_LM(N=N, X=X, beta_md=beta_md, y=y, _lambda=_lambda, a=a)
...
#         L = _np.max(_np.array([L_convex, 1./(gamma)]))
        ...
        while ((not converged) or (k<3)) and k <= maxit:
            ...
            speed_norm = _np.linalg.norm(beta_md - beta_md_old, ord=2) # restarting
            converged = (_np.linalg.norm(beta_md - beta_md_old, ord=_np.infty) < tol)
            smooth_grad = _update_smooth_grad_MCP_LM(N=N, X=X, beta_md=beta_md, y=y, _lambda=_lambda, gamma=gamma)
            beta = soft_thresholding(x=beta - opt_lambda*smooth_grad, lambda_=opt_lambda*_lambda)
            beta_ag = soft_thresholding(x=beta_md - opt_beta*smooth_grad, lambda_=opt_beta*_lambda)
#             converged = _np.all(_np.max(_np.abs(beta_md - beta_ag)/opt_beta) < tol).item()
#             converged = (_np.linalg.norm(beta_md - beta_ag, ord=_np.infty) < (tol*opt_beta))
...
@_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def solution_path_LM(design_matrix, outcome, lambda_, beta_0 = _np.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2., add_intercept_column=True):
    '''
    Carry out the optimization for the solution path without the strong rule.
    '''
    #     add design matrix column for the intercept, if it's not there already
    if add_intercept_column == True:
        if _np.any(design_matrix[:,0] != design_matrix[0,0]): # check if design matrix has included a column for intercept or not
            intercept_design = _np.ones(N).reshape(-1, 1)
...
@_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def _UAG_LM_SCAD_MCP_strongrule(design_matrix, outcome, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2., L_convex=1.1, add_intercept_column = True, strongrule=True):
    '''
...
#     add design matrix column for the intercept, if it's not there already
    ...
    if strongrule == True:
        ...
        elim = _strong_rule_seq_LM(X, y, beta_old=_np.zeros(p_original), lambda_new=_lambda, lambda_old=_lambda_max)
...
@_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def UAG_LM_SCAD_MCP_strongrule(design_matrix, outcome, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2., L_convex=1.1, add_intercept_column = True, strongrule=True):
    """
...
@_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def solution_path_LM_strongrule(design_matrix, outcome, lambda_, beta_0 = _np.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2., add_intercept_column=True):
    '''
    ...
    for j in range(len(lambda_)-1):
        ...
        _new_beta[_np.logical_not(elim)] = UAG_LM_SCAD_MCP(design_matrix=_design_matrix[:, _np.logical_not(elim)], outcome=outcome, beta_0 = _beta_0[_np.logical_not(elim)], tol=tol, maxit=maxit, _lambda=lambda_[j], penalty=penalty, a=a, gamma=gamma, add_intercept_column=False)[1]
...
#     y_temp /= _np.std(y)
    ...
    grad_at_0 = _memmap_update_smooth_grad_convex_LM(N=N, p=p, X=X, beta_md=_np.zeros(p), y=y, _dtype=_dtype, _order=_order)
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def memmap_UAG_LM_SCAD_MCP(design_matrix, outcome, N, p, L_convex, _dtype, _order, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2.):
    '''
    Carry out the optimization for penalized LM for a fixed lambda.
    '''
    X = design_matrix
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def memmap_solution_path_LM(design_matrix, outcome, lambda_, L_convex, N, p, beta_0 = _np.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2., _dtype='float32', _order="F"):
    '''
    Carry out the optimization for the solution path without the strong rule.
    '''
...
def _update_smooth_grad_SCAD_logistic(N, X, beta_md, y, _lambda, a):
    ...
    Update the gradient of the smooth objective component for SCAD penalty.
    '''
    return _update_smooth_grad_convex_logistic(N=N, X=X, beta_md=beta_md, y=y) + SCAD_concave_grad(x=beta_md, lambda_=_lambda, a=a)
...
def _update_smooth_grad_MCP_logistic(N, X, beta_md, y, _lambda, gamma):
    ...
    Update the gradient of the smooth objective component for MCP penalty.
    '''
    return _update_smooth_grad_convex_logistic(N=N, X=X, beta_md=beta_md, y=y) + MCP_concave_grad(x=beta_md, lambda_=_lambda, gamma=gamma)
...
def _strong_rule_seq_logistic(X, y, beta_old, lambda_new, lambda_old):
    ...
    Use sequential strong to determine which betas to be nullified next.
    """
    grad = _np.abs((y-_np.tanh(X@beta_old/2)/2-.5)@X_temp/(2*len(y)))
    eliminated = (grad < 2*lambda_new - lambda_old) # True means the value gets eliminated
    eliminated = _np.hstack((_np.array([False]), eliminated)) # because intercept coefficient is not penalized
    return eliminated

@_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def UAG_logistic_SCAD_MCP(design_matrix, outcome, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2., L_convex=1.1, add_intercept_column = True):
    '''
...
#         L = _np.max(_np.array([L_convex, 1./(a-1)]))
        ...
        while ((not converged) or (k<3)) and k <= maxit:
            ...
            speed_norm = _np.linalg.norm(beta_md - beta_md_old, ord=2) # restarting
            converged = (_np.linalg.norm(beta_md - beta_md_old, ord=_np.infty) < tol)
            smooth_grad = _update_smooth_grad_SCAD_logistic(N=N, X=X, beta_md=beta_md, y=y, _lambda=_lambda, a=a)
            beta = soft_thresholding(x=beta - opt_lambda*smooth_grad, lambda_=opt_lambda*_lambda)
            beta_ag = soft_thresholding(x=beta_md - opt_beta*smooth_grad, lambda_=opt_beta*_lambda)
#             converged = _np.all(_np.max(_np.abs(beta_md - beta_ag)/opt_beta) < tol).item()
#             converged = (_np.linalg.norm(beta_md - beta_ag, ord=_np.infty) < (tol*opt_beta))
...
#         L = _np.max(_np.array([L_convex, 1./(gamma)]))
        ...
        while ((not converged) or (k<3)) and k <= maxit:
            ...
            speed_norm = _np.linalg.norm(beta_md - beta_md_old, ord=2) # restarting
            converged = (_np.linalg.norm(beta_md - beta_md_old, ord=_np.infty) < tol)
            smooth_grad = _update_smooth_grad_MCP_logistic(N=N, X=X, beta_md=beta_md, y=y, _lambda=_lambda, gamma=gamma)
            beta = soft_thresholding(x=beta - opt_lambda*smooth_grad, lambda_=opt_lambda*_lambda)
            beta_ag = soft_thresholding(x=beta_md - opt_beta*smooth_grad, lambda_=opt_beta*_lambda)
#             converged = _np.all(_np.max(_np.abs(beta_md - beta_ag)/opt_beta) < tol).item()
#             converged = (_np.linalg.norm(beta_md - beta_ag, ord=_np.infty) < (tol*opt_beta))
...
@_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def solution_path_logistic(design_matrix, outcome, lambda_, beta_0 = _np.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2., add_intercept_column=True):
    '''
    Carry out the optimization for the solution path without the strong rule.
    '''
    #     add design matrix column for the intercept, if it's not there already
...
@_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def _UAG_logistic_SCAD_MCP_strongrule(design_matrix, outcome, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2., L_convex=1.1, add_intercept_column = True, strongrule=True):
    '''
    Carry out the optimization for a fixed lambda with strong rule.
    '''
    X = design_matrix.copy()
...
#     add design matrix column for the intercept, if it's not there already
    ...
    if strongrule == True:
        ...
        p_original = X.shape[1]
        elim = _strong_rule_seq_logistic(X, y, beta_old=_np.zeros(p_original), lambda_new=_lambda, lambda_old=_lambda_max)
        X = X[:, _np.logical_not(elim)]
        beta = beta[_np.logical_not(elim)]
...
@_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def UAG_logistic_SCAD_MCP_strongrule(design_matrix, outcome, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2., L_convex=1.1, add_intercept_column = True, strongrule=True):
    """
...
@_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def solution_path_logistic_strongrule(design_matrix, outcome, lambda_, beta_0 = _np.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2., add_intercept_column=True):
    '''
    ...
    elim = _np.array([False]*_design_matrix.shape[1])
    for j in range(len(lambda_)-1):
        _elim = _strong_rule_seq_logistic(X=_design_matrix, y=outcome, beta_old=beta_mat[j,:], lambda_new=lambda_[j+1], lambda_old=lambda_[j])
        elim = _np.logical_and(elim, _elim)
        _beta_0 = beta_mat[j,:]
        _new_beta = _np.zeros(_design_matrix.shape[1])
        _new_beta[_np.logical_not(elim)] = UAG_logistic_SCAD_MCP(design_matrix=_design_matrix[:, _np.logical_not(elim)], outcome=outcome, beta_0 = _beta_0[_np.logical_not(elim)], tol=tol, maxit=maxit, _lambda=lambda_[j], penalty=penalty, a=a, gamma=gamma, add_intercept_column=False)[1]
        beta_mat[j+1,:] = _new_beta
    return beta_mat[1:,:]
...
#     y_temp /= _np.std(y)
    ...
    grad_at_0 = _memmap_update_smooth_grad_convex_logistic(N=N, p=p, X=X, beta_md=_np.zeros(p), y=y, _dtype=_dtype, _order=_order)
    lambda_max = _np.linalg.norm(grad_at_0[1:], ord=_np.infty)
    return lambda_max
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def memmap_UAG_logistic_SCAD_MCP(design_matrix, outcome, N, p, L_convex, _dtype, _order, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2.):
    '''
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def memmap_solution_path_logistic(design_matrix, outcome, lambda_, L_convex, N, p, beta_0 = _np.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2., _dtype='float32', _order="F"):
    '''
    Carry out the optimization for the solution path without the strong rule.
    '''
    beta_mat = _np.zeros((len(lambda_)+1, p))
...
#######################################################################################
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def _memmap_update_smooth_grad_convex_logistic_parallel(N, p, X, beta_md, y, _dtype, _order, chunck_size):
    '''
    Update the gradient of the smooth convex objective component.
    '''
    _itemsize = _np.dtype(_dtype).itemsize
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def _memmap_update_smooth_grad_SCAD_logistic_parallel(N, p, X, beta_md, y, _lambda, a, _dtype, _order, chunck_size):
    '''
    Update the gradient of the smooth objective component for SCAD penalty.
    '''
    return _memmap_update_smooth_grad_convex_logistic_parallel(N=N, p=p, X=X, beta_md=beta_md, y=y, _dtype=_dtype, _order=_order, chunck_size=chunck_size) + SCAD_concave_grad(x=beta_md, lambda_=_lambda, a=a)

# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def _memmap_update_smooth_grad_MCP_logistic_parallel(N, p, X, beta_md, y, _lambda, gamma, _dtype, _order, chunck_size):
    '''
    Update the gradient of the smooth objective component for MCP penalty.
    '''
    return _memmap_update_smooth_grad_convex_logistic_parallel(N=N, p=p, X=X, beta_md=beta_md, y=y, _dtype=_dtype, _order=_order, chunck_size=chunck_size) + MCP_concave_grad(x=beta_md, lambda_=_lambda, gamma=gamma)


# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def memmap_lambda_max_logistic_parallel(X, y, N, p, _dtype, _order, chunck_size=50000):
    """
    Calculate the lambda_max, i.e., the minimum lambda to nullify all penalized betas.
    """
#     X_temp = X.copy()
...
#     y_temp /= _np.std(y)
    ...
    grad_at_0 = _memmap_update_smooth_grad_convex_logistic_parallel(N=N, p=p, X=X, beta_md=_np.zeros(p), y=y, _dtype=_dtype, _order=_order, chunck_size=chunck_size)
    lambda_max = _np.linalg.norm(grad_at_0[1:], ord=_np.infty)
    return lambda_max
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def memmap_UAG_logistic_SCAD_MCP_parallel(design_matrix, outcome, N, p, L_convex, _dtype, _order, beta_0 = _np.ones(1), tol=1e-2, maxit=500, _lambda=.5, penalty="SCAD", a=3.7, gamma=2., chunck_size=50000):
    '''
    Carry out the optimization for penalized logistic for a fixed lambda.
    '''
    X = design_matrix
...
#         L = _np.max(_np.array([L_convex, 1./(a-1)]))
        ...
        while ((not converged) or (k<3)) and k <= maxit:
            ...
            speed_norm = _np.linalg.norm(beta_md - beta_md_old, ord=2) # restarting
            converged = (_np.linalg.norm(beta_md - beta_md_old, ord=_np.infty) < tol)
            smooth_grad = _memmap_update_smooth_grad_SCAD_logistic_parallel(N=N, p=p, X=X, beta_md=beta_md, y=y, _lambda=_lambda, a=a, _dtype=_dtype, _order=_order, chunck_size=chunck_size)
            beta = soft_thresholding(x=beta - opt_lambda*smooth_grad, lambda_=opt_lambda*_lambda)
            beta_ag = soft_thresholding(x=beta_md - opt_beta*smooth_grad, lambda_=opt_beta*_lambda)
#             converged = _np.all(_np.max(_np.abs(beta_md - beta_ag)/opt_beta) < tol).item()
#             converged = (_np.linalg.norm(beta_md - beta_ag, ord=_np.infty) < (tol*opt_beta))
...
#         L = _np.max(_np.array([L_convex, 1./(gamma)]))
        ...
        while ((not converged) or (k<3)) and k <= maxit:
            ...
            speed_norm = _np.linalg.norm(beta_md - beta_md_old, ord=2) # restarting
            converged = (_np.linalg.norm(beta_md - beta_md_old, ord=_np.infty) < tol)
            smooth_grad = _memmap_update_smooth_grad_MCP_logistic_parallel(N=N, p=p, X=X, beta_md=beta_md, y=y, _lambda=_lambda, gamma=gamma, _dtype=_dtype, _order=_order)
            beta = soft_thresholding(x=beta - opt_lambda*smooth_grad, lambda_=opt_lambda*_lambda)
            beta_ag = soft_thresholding(x=beta_md - opt_beta*smooth_grad, lambda_=opt_beta*_lambda)
#             converged = _np.all(_np.max(_np.abs(beta_md - beta_ag)/opt_beta) < tol).item()
#             converged = (_np.linalg.norm(beta_md - beta_ag, ord=_np.infty) < (tol*opt_beta))
...
# @_jit(nopython=True, cache=True, parallel=True, fastmath=True, nogil=True)
def memmap_solution_path_logistic_parallel(design_matrix, outcome, lambda_, L_convex, N, p, beta_0 = _np.ones(1), tol=1e-2, maxit=500, penalty="SCAD", a=3.7, gamma=2., _dtype='float32', _order="F"):
    '''
    Carry out the optimization for the solution path without the strong rule.
    '''
    beta_mat = _np.zeros((len(lambda_)+1, p))
    for j in range(len(lambda_)):
        beta_mat[j+1,:] = memmap_UAG_logistic_SCAD_MCP_parallel(design_matrix=design_matrix, outcome=outcome, N=N, p=p, L_convex=L_convex, beta_0 = beta_mat[j,:], tol=tol, maxit=maxit, _lambda=lambda_[j], penalty=penalty, a=a, gamma=gamma, _dtype=_dtype, _order=_order,)[1]
    return beta_mat[1:,:]
