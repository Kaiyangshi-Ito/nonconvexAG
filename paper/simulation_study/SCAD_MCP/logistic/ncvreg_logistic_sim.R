...
lambda_max_logistic= function(X, y){
  X[,-1] = scale(X[,-1])
  grad_at_0 = abs((y-mean(y))%*%X/(length(y))) #  I removed .5 because it's 1/N not 1/2N for ncvreg
  lambda_max = max(grad_at_0[-1])
  return(lambda_max)
}
...
for (i in c(1:4)){
  for (j in c(1:3)){
    for (seed in c(1:100)){
      ...
      lambda_seq = seq(lambda_max_logistic(X_sim, y_sim), 0, length.out=50+1)
      temp_fit = ncvreg(data.frame(X_sim[,-1]), y_sim, family='binomial', penalty = "SCAD",lambda=lambda_seq[-51])
      temp_beta = temp_fit$beta
      testing_X = SCAD_sim[i,j,seed,1001:2000,-1]
      testing_y = SCAD_sim[i,j,seed,1001:2000,1]
      testing_error = -t(testing_y)%*%testing_X%*%temp_beta+colSums(log(exp(testing_X%*%temp_beta)+1))
...
for (i in c(1:4)){
  for (j in c(1:3)){
    for (seed in c(1:100)){
      ...
      lambda_seq = seq(lambda_max_logistic(X_sim, y_sim), 0, length.out=50+1)
      temp_fit = ncvreg(data.frame(X_sim[,-1]), y_sim, family='binomial', penalty = "MCP",lambda=lambda_seq[-51])
      temp_beta = temp_fit$beta
