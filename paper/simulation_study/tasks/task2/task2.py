...
# ## performance of signal recovery ($2-$norm, $\infty-$norm, PPV/NPV) based on best testing set performance, with SNR=$1,3,7,10$, Toeplitz being $0.1,0.5,0.9$, and N=$1000$, p=$2050$ with each $10$ coefficients simulated from $N(.5,1),N(.5,1),N(-.5,1),N(-.5,1),N(1,1)$, sparsely located in the array, with $500$ zeros in-between 
# 
...
for (seed in 1:100){
set.seed(seed)
true_beta = c(rnorm(10, .5, 1), rep(0,500), rnorm(10, .5, 1), rep(0,500), rnorm(10, -.5, 1), rep(0,500), rnorm(10, -.5, 1), rep(0,500), rnorm(10, 1, 1))
...
y_temp = plogis(y_temp)
y_temp = rbinom(2000,1,y_temp)
x[i,j,seed,,2] = 1
x[i,j,seed,,1] = y_temp
}
}
...
for i, j, seed in itertools.product(range(4), range(3), range(100)):
    ...
    lambda_seq = cp.linspace(lambda_max_logistic(X_sim, y_sim),
                             0,
                             num=50,
                             endpoint=False)
    temp_beta = fit_logistic(X_sim,
                             y_sim,
    ...
    testing_error = -testing_y.T @ testing_X @ temp_beta.T + cp.sum(
        cp.logaddexp(testing_X @ temp_beta.T, 0), 0)
...
# """)
# print("simulating from R done!")
MCP_sim = SCAD_sim.copy()
MCP_true_beta = np.array(robjects.r["true_betas"])
